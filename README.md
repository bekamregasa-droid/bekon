# Bekon: Career Discovery App

Bekon is a mobile app I built to help students figure out career paths that actually fit how they think. Instead of boring text surveys, it uses short interactive games to measure cognitive strengths. Then match users with careers that fit with their natural abilities.

---

## It is  Built With:

* **Framework:** Flutter & Dart
* **Backend:** Firebase (Auth & Firestore)
* **Design:** Custom animations and mobile-first UI

---

##  Key Features

* **Onboarding & Auth:** Intro screens with secure sign-up/login.
* **6 Brain Assessment Games:** Quick minigames that test memory, pattern recognition, and how fast you solve problems.
* **Smart Career Matching:** Recommends specific career paths based on performance data.
* **Visual Analytics:** Easy-to-read breakdowns of strengths.
* **Results:** Clear visualizations showing core strengths and career matches.
* **Career Explorer:** Dives into recommended job and industries.
* **Settings and account:** You can change the appearance and log out 
---

##  Download & Test the App

You can download the pre-compiled Android APK directly to test the app.

*  [**Download Bekon APK (Google Drive)**] https://drive.google.com/drive/folders/1mgV9X1eO_0vDA0HplBMB7v3fCa7ATogX

---

## To Run the Project

### What You Need

* Flutter SDK (>= 3.0.0)
* Dart SDK (>= 3.0.0)
* Android Studio or VS Code
* An emulator or physical device


1. **Clone the repo:**
   ```bash
   git clone [https://github.com/bekamregasa-droid/bekon.git](https://github.com/bekamregasa-droid/bekon.git)
   cd bekon
